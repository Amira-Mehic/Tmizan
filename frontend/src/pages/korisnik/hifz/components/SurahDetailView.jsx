...
import { toArabicNumerals, fmtDate, todayStr, getSurahsForPage } from "../../../../constants/hifz/helpers";
...
export function SurahDetailView({ surah, pageStatuses, surahData, onSave, onBack, onOpenPage, theme, s }) {
  const d        = surahData || {};
  ...
  // ── Boje hero card-a (aggregated) ───────────────────────────────────────
  const heroSt = pct === 100
    ? STATUS.savladano
    : pct > 0
    ? STATUS.u_toku
    : STATUS.prazna;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* ── HERO CARD ───────────────────────────────────────────────────────── */}
      <div className={`rounded-2xl border p-4 sm:p-5 ${heroSt.bg} ${heroSt.border}`}>
            ...
            return (
              <button key={p}
                ...
                className={`group relative flex flex-col rounded-2xl border p-3 sm:p-4 text-left transition-all
                  ...
                  ${isActive ? `${stObj.bg} ${stObj.border}` : `${tCardAlt} ${tBorder}`}`}
                style={{ minHeight: "140px" }}
              >
                {/* Broj stranice + arapski */}
                <div className="flex items-baseline justify-between mb-2">
                  <span className={`text-2xl sm:text-3xl font-black leading-none ${isActive ? stObj.text : tText}`}>{p}</span>
                  <span className={`text-base opacity-25 ${tMuted}`} style={{ fontFamily: "'Amiri', serif" }}>
                    {toArabicNumerals(p)}
                  </span>
                </div>
                ...
                {/* Ime sure */}
                {mainSura && (
                  <span className={`text-[10px] font-semibold mb-1 leading-tight ${isActive ? stObj.text : tSubtle} opacity-80 line-clamp-1`}>
                    {mainSura.name}
                  </span>
                )}
                ...
                {/* Ajeti */}
                {vc && (
                  <span className={`text-[10px] ${isActive ? stObj.text : tSubtle} opacity-60 mb-auto`}>
                ...
                <div className="flex items-center gap-1.5 mt-2">
                  <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${stObj.dot}`} />
                  <span className={`text-[10px] font-semibold ${isActive ? stObj.text : tSubtle}`}>
                    {statusLabelF}
                  </span>
                </div>
